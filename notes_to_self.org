...
Notes from meeting:
I'm rescoring episodes marked as REM to wake, if they are preceeded by at least 3 epochs of wake in a row. 
I'm also counting how many time this happens as part of the automated scoring. 
Jonathan said that this almost never happens in normal humans or rodents.  When it does it is only in pathological 
cases (narcolepsy and sleep apnea, when people are chronically sleep deprived)

Implementing this change had a positive impact on every agreement parameter for BA121240.txt (kappa is better, etc.)
Running all the BA files in compareto2sec all of the agreement params are slightly better or the same.  

I have moved all the files in place to do sleep reports.  Run the PCASCOREBATCHMODE.m on each of the two directories. 
They are in data/mrempe/autoscore_and_epoch_length_study_data/2-Second_epochs and 10-Second_epochs
The only one that is weird is BA1218.  There are two separate files since there is a gap of about 3 hours between them.  
I'm only waiting on BL1194 in the 2-second bins.  Then I can run all the sleep reports and send them to Jonathan.  

TO DO:

1 [X]Code up this rule about re-scoring epochs marked as REM
2 [X] Count the number of epochs that get rescored as wake due to this rule
3 [X] Compare the agreement statistics if I use this rule or I don't.  For BA they slightly improved
3a [X] Make sure I have all the latest files from glitch/sleep_data/Lactate Strain Comparison, Part1/2-Sec Epoch Data and 10-Sec Epoch Data. 
Will said the data files in 10-sec Epoch data are the best and they are the ones I should use.  Will was still working on a few of these today.  
4 Batch autoscore all the 10-sec files for BA and BL 10 sec epochs that correspond to the BA and BL files we have scored at 2 sec epochs
5 Batch autoscore all the 2-sec files for BA and BL
6 Combine all the BA, BL human and machine scored 10-sec epoch files into one directory
7 Combine all the BA, BL human and machine scored 2-sec epoch files into one directory
8 Run a sleep report on all the 10-sec epoch data using 360 epochs in one interval (BA and BL, human and machine scored)
9 Run a sleep report on all the 2-sec epoch data using 180 epochs in one interval (BA and BL, human and machine scored)
10 Run a sleep report on all the 10-sec data using 8640 epochs in one interval (BA and BL, human and machine scored)
11 Run a sleep report on all the 2-sec data using 8640 epochs in one interval (BA and BL, human and machine scored)


* Thursday, Sept. 11, 2014
Run the sleep reports with 12-minute bins for both the 10-second epoch data and the 2-second epoch data. 
For the 10-sec epoch data this means 72 epochs per bin.  For the 2-second epoch data this is 360 epochs/bin.

BA1203 in 10-second bins is causing problems.  Every epoch is getting scored as wake. I think this is why: This file 
starts at about midnight where the other files start at 8 PM. I wonder if I'm using the wrong chunk of time for 
training data (so maybe every epoch in my training set is wake).  Will re-output this file and it looks good now. 

File BA1218A.txt and BA1218B.txt caused problems.  I think it was not able to find the time 2PM in that 
dataset.  These are the two with the 3 hour gap between them.  I am leaving them out of the analysis for now.  

How does SLEEPREPORT.m handle a datafile that is much shorter than all the others? 

check BL1180.  EEG1 looked bad.  EEG2 too?  EEG2 is fine. 

The 8th file has a problem. BA1214.txt kappa=0.sleep_agree=0, It was an issue with the EMG signal.  It is fixed now.  
I fixed BL1180-2sec.txt too.  BA1214-2sec.txt looks fine. 
I fixed EMG on BA1205.txt too.  Check files for EMG signal?  10-sec and 2-sec? 
10-sec files:       2-sec files:
BA1203 OK     OK
BA1204 OK     OK
BA1205 OK      OK
BA1208 Fixed   OK
...
SLEEPREPORT is crashing on the 2-second data, but I think that is because there are several epochs that 
are unscored so the second column of textdata is empty.  Should I only run the sleep report on the 
machine-scored versions of the 2-second data since the human-scored two second epoch files were only 
scored for a few hours.  

* Thursday, Sept. 18, 2014
Notes from talk with Jonathan:  Most of the sleep statistics look really good comparing human scored vs. 
machine scored.  However, the bout durations are way off.  It could be that the machine scoring is not 
catching the epochs that a human scorer sees as partially wake and therefore classifies the entire 
epoch as wake.  The machine scoring would call the entire section sleep meaning SWS duration would 
be much longer. 

TO DO: run the homeostatic model on the human scored and machine scored for the 10-second data to make 
sure that the time constants are the same.  We want to know if those brief awakenings that the machine 
learning is not catching are not affecting the homeostatic model fit.  

SLEEPREPORT.m is crashing for the 2-second data because for some files there is missing sleep state. 
This is to be expected since the 2-second files only had a subset scored by hand and sometimes the other 
epochs are left blank.  How should we modify the SLEEPREPORT algorithm to handle the case where the sleep 
state is left blank for an entire interval?  I coded in a quick fix for this.  If the sleep state was empty 
for all the rows, I just set up a vector of empty elements. 

Plan for the rest of today:  Make two plots: one is taui automated vs. taui human.  One plot is taud automated 
vs. taud human.  Use different symbols for different strains.
Put all of the data in spreadsheets:  taui 10sec delta auto, taui 10sec delta human, 
                                      taui 10sec lact  auto, taui 10sec lact  human,
                                      taud 10sec delta auto, taud 10sec delta human, 
                                      taud 10sec lact  auto, taud 10sec lact  human
                                      taui 2sec delta auto, taui 2sec delta human, 
                                      taui 2sec lact  auto, taui 2sec lact  human,
                                      taud 2sec delta auto, taud 2sec delta human, 
                                      taud 2sec lact  auto, taud 2sec lact  human

One Sheet for 10sec, one sheet for 2 sec?  rows are recordings, columns are taui delta auto, taui delta human, taui la  


I saved what I have to a matlab workspace called tau_compare_auto.mat. 
               

BEFORE NEXT THURSDAY:  find the best and worst agreement between auto-scored files and human-scored files so we can look 
at the EEG and EMG of those animals to understand what contributes to good autoscoring vs. bad autoscoring. 

* Thursday, Sept. 25 2014
What is going on with BL1186? Autoscored gives me a lactate Taui of 2.3e12.  Looking at the SleepPctFFT report, 
wake minutes are almost identical between human scored and machine scored.  REM minutes are 80 by hand, vs 72 machine scored
SWS minutes are 476 by hand and 529 machine scored.  totals agree

This file does just fine if I use the brute force method.  The model fit between autoscored and human scored look 
very similar and the tau values are a little off, but not by much.  Looking at the contour plots it looks like 
maybe there is a second local minimum way up on the taui axis and maybe NM is finding that one instead of the relevant 
one with small values of the taus.  Re-run this file with Nelder-Mead and new starting guesses to make sure it converges 
to the right values.  Then paste the new values into the spreadsheet for Jonathan. taui=0.209 taud=0.1826.
That was exactly what was going on.  Nelder-Mead was finding a different local minimum, way up on the taui axis. 
With better starting values it converged to the correct values.  
I have modified the code to use smaller starting guesses if lactate is the signal.  

Running the model on the 2-sec data, I find that BL1182-2secAUTOSCORED.txt gives a model fit that is just 
a straight line. Making the tau values huge.  This could be a case of NM finding the wrong minimum again.  
Changing starting guesses to [1 1] didn't help either.  The manually-scored file does just fine. 
Try running these two files (BL1182-2sec manual and auto) using the brute force method. 
Using Brute Force I see that the human-scored data has a nice global minimum in a reasonable range, but the 
autoscored data doesn't have a minimum there.  So even the Brute Force method doesn't find a reasonable minimum.
The autoscoring of this one file must have altered it in such a way that the model curve-fitting no longer 
has a nice minimum. 

I re-ran the classifying algorithm and found that for this file the first 3 components don't explain as 
much of the data as others.  There are non-zero numbers in the last 4 Principal Components.  
It looks like it's really having trouble with the REM in this file.  There is a lot of it and it's not getting scored 
well.  Perhaps the subset scored by a human didn't have much REM?  Error from classify is low, so of the subset scored, 
nearly all of them are being scored the same by the algorithm as compared to the human. 

I tried a random forest and it wasn't much better. 

For this one file (BL1182-2secAUTOSCORED.txt) comparing the autoscored model fit to the manually scored model fit, 
it looks like the model is going the wrong direction (up during sleep and down during wake).  There is a long 
section 5 hours into the file that is almost entirely wake, but the SWA model is going down for those hours.  Comparing 
the graphs of SWA model fit between autoscored and human scored they are almost inversions.  Every time human-scored
goes up autoscored goes down and vice-versa. 

Compare sleep states between autoscored and human scored.  Are the numbers getting off somehow?  
Running the model using delta1 it works just fine autoscored and manually scored.  What is different 
between delta1 and delta2?  And didn't I use EEG2 in the autoscoring?  

For now I'm using delta1 for BL1182-2secAUTOSCORED.txt and it seems to give good output.  I have no idea 
why it isn't working if I use delta2.  The plotted circles look the same, why can't the model fit them for 
delta2 if it works for delta1?  

* Thursday, Oct. 2 2014
I'm re-running the model for SWA and lactate auto scored and human scored for BA1216 with 2 second epochs. 
Replace all the relevant entries in TauiTauDByStrainScoretypeEpochDuration.xlsx 

I modified PROCESSLBATCHMODE.m to use a larger window in the median filter if epoch length is shorter. 
This smoothed out all the artefacts in the lactate signal, but had little effect on the tau values. 

Modify PROCESSLBATCHMODE.m to compute the residual error for each run.  Run it on all combos of epoch, strain, and 
signal. 
Don't forget to redo BL1182-2sec AUTO and MANUAL? using delta1 instead of delta2. 

I'm running tau_script.m now.  Should I modify how I compute the residual error?  Should it be normalized in 
some way?  I'm thinking that since the units are so different between delta and lactate perhaps we should 
normalize to the range of the data (delta or lactate)

* Thursday, October 9, 2014
NOTES from morning meeting: Work on incorporating JW's changes and look at making one more figure (or panel).
In each strain identify the animal with best agreement and worst agreement and open neuroscore and 
check these two.  We expect the dynamic range of EEG or EMG to predict the effectiveness of the machine 
scoring.  We may systematically remove animals with low dynamic range and recheck the agreement parameters (kappa, etc.)

          10-Second                   2-second  
          best    worst               best    worst
BA        BA1204  BA1217              BA1212  BA1213
BL        BL1180  BL1193              BL1180  BL1183

For the 2-second data in this table I restricted the autoclassification to be just the time from 10AM-2PM (the time that was scored by hand).
 
 * Thursday, October 16, 2014
 Working on paper edits:  
 [X] Get rid of individual statistics in plotly figure
 [X] Fix the x label (percentage)



 * Thursday, October 23, 2014
 Notes from morning meeting: We may want to leave epochs marked as artefact out of the training for the auto-scoring 
 procedure.  Right now it looks like I handle artefacts first (using some simple rules) and then feed everything to 
 the autoscoring procedure.  So before I even start doing the PCA stuff every epoch is scored as wake, REM, or SWS. 

 To follow up on the notes from Oct. 9:  

Paper thoughts:  It doesn't seem like we are really showing the agreement stats for the autoscoring that we did for entire data 
files.  We have the two panels in there for the 8640 epochs, but we don't show kappa and agreement stats for entire files, 10-second
 or 2-second.  I should include the boxplots for the 10-second and 2-second data.  I HAVE A DRAFT OF THIS FIGURE IN THE MANUSCRIPT.
 Also, Figure 1 shows that we need to use about 20-40% of the datafile as training data.  How much are we using now?  10AM-2PM, so I think about   

 Make a boxplot where the columns are BA 10-sec, B6 10-sec, BA 2-sec, B6 2-sec and each column has 5 small boxes (wake, SWS, REM, global,kappa)
 In plotly make the column names numbers.  This is how you alter the horizontal spacing (and grouping). For instance, call the first column 1 
 and the second column 1.1 and the third column 3.  This will give uneven spacing. Each column then holds data for each box. 
 I'll probably need to run the code again to compute kappa and agreement params for all the data (BA, BL, 2-sec, 10-sec) 

 * Thursday, November 6, 2014
 TO DO: From talk with Janne: In ProcessLBATCHMODE.m remove the code that cuts off data before 8PM and add a parameter to restrict the 
 data to be of the following categories:
 baseline: 10AM-10AM (first 24 hours of recording)
 SD:       10AM-4PM  
 recovery: 4PM-end of recording
 I HAVE THIS CODED IN PROCESSLBATCHMODE.M BUT IT CRASHED ON ONE FILE USING BASELINE.  IT MAY BE THE FILE OR IT MAY BE THE CODE
 This way we can compare tau values for the different phases of experiment.  
 This should be a new branch of the code in GitHub.  Mess with the windows GitHub client?  

 TO DO: make a figure of kappa or global agreement (y-axis) vs dynamic range (x-axis).  One for lactate (both strands) and one for 
 SWA (both strands).  What about epoch length?  include both 10-second and 2-second epochs?   I only included 10-seconds because 2-seconds 
 are only scored for 8640 epochs. 

 Dynamic Range of Delta activity and lactate showed no correlation with kappa and global agreement.  
 Check other ranges (theta, etc) inside classify_usingPCA.m rather than PROCESSLBATCHMODE.m 
 No trend using theta on BA (10 second)
 No trend using theta on BL (10 second)
 perhaps a slight downward trend using low beta on BA
 Somewhat of an inverse trend using low beta on BL (higher dynamic range gave lower kappa)
 No trend using high beta on BA
 slight inverse trend using high beta on BL
 slight inverse trend using EMG on BA
 slight inverse trend using EMG on BL
 no trend in ratio of theta to delta on BA
 no trend in ratio of theta to delta on BL
 no trend in feature 7 in BA
 slight inverse trend in feature 7 in BL

 * Thursday, November 20, 2014
 To do today: 
  [X] Compute and save residuals for SWA and lactate for both strains.  Email them to myself or something so I can work on them later. 
  [X] Copy the code and modify the model so it is linear and not exponential.  What are good starting values for slopes?  Code in linear_version 
  folder.  
  [] Code up a version of the model that uses theta (5-9Hz) during wake rather than delta during sleep. And a version that uses 
  theta during wake in addition to delta during sleep.   
  [] run residuals_BF_script.m to compute residuals for SWA and lactate using Brute Force.  Residuals computed using NM are saved in 
  residuals_for_Frontiers_paper.mat.
  [X] run autoscoring for Will on \\fs1/wisordata/Clegern/Dual Cannula Lactate/ DC-001 DC-002 DC-003 DC-004 

  Residuals were computed in the following way: square root of sum of (computed-data)^2 divided by number of data points (SWS episodes or number of 
  lactate data points)

1:37 I found a bug!  I was running the linear model, but the final plot used the exponential model, so it gave a really bad fit.  Now it seems to be 
working.  Check different starting values.  The exponential model seems to be a much better fit.  
1:49 The linear model seems to be working fine now for delta. Some plots look nearly as good as the exponential model, while some look quite a bit 
worse.  That will help our case. Try it with lactate too.  It works for lactate too.  Linear isn't too bad for lactate, but the exponential model 
seems to be better, especially for a couple of them.  Compute residuals and compare averages along with comparing individual values showing that 
all residuals go down or stay the same when going from a linear model to the exponential model.  


* Thursday, Jan. 8 2015
I'm working on checking some final bits before we submit the autoscoring paper to SLEEP.  
When working on the revisions I thought I found that the taui and taud values were not matched up properly with the file names in a spreadsheet. 
I re-ran the simulations and the file total_sleep_total_wake_totalREM.xlsx has the correct values.  Which spreadsheets were used for the paper? 

* Tuesday, January 13, 2015
Code up the scaled lactate and make a plot of scaled lactate vs. time using human scored and machine scored.  10-second epochs or 2-second epochs? 

I had to comment out lines 246-250 of PROCESSLBATCHMODE.m to make it work.  What was happening was the code that cuts off the signal before the second 
NREM episode of 1 min or longer was making the autoscored file have a slightly different length than the human scored file.  This made it hard to 
line them up accurately.  I commented out these lines.  UNCOMMENT THEM!!  I uncommented them.  

I save the matlab workspace as scaled_L_workspace.mat.  I can use this to make a basic figure.  But, I think it would be instructive to also plot the raw lactate
data because the machine-scored version actually does a better job than the human-scored version and it would be nice to show this.  I added this and saved the workspace. 

* Monday, January 19, 2015
Don't forget to uncomment lines 246-250 of PROCESSLBATCHMODE.m 

Lines 303 and 304 of PROCESSLBATCHMODE.m had LowA{FileCounter} = LAnormalized;  %normalized LA UppA{FileCounter} = UAnormalized; before  CHANGE BACK!!

* Monday, February 2, 2015
I'm writing a script, whisking_data_script.m, to output data to an excel file.  So far I have tested the writing to the excel 
file and it seems to be working.  Now to do: update PROCESSLBATCHMODE.m to accept a 2 element vector for the restrict input.  If 
it has two elements, restrict the recording to be only between these two times (measured as hours from the beginning of the recording).
And write all the calls to PROCESSLBATCHMODE into the script.  

inputs to PROCESSLBATCHMODE.m can now be delta1, delta2, EEG1, or EEG2.

restrict seems to be working with a 2-element vector now.  

FIX BUG WHERE EEG2 doesn't work!! Use JW's code to make the reading in more robust.  DONE!

when running Janne's datasets using EEG2, files 2011_T and 2178_S cause NM to time out.  I re-ran them using brute force and there just wasn't a minimum at all.
For now just leave those in there and put a N/A or something in the spreadsheet.  
Another problem:  for the whisking time, there are very few episodes of SWS, so the code is complaining when I try to find UA and LA.  For now, just exclude that 
chunk of time from the script.  Until we can figure something out.  

Include some notes here from Thursday?  

* Thursday, February 5, 2015
I'm working on running PROCESSLBATCHMODE.m on the whisking data for short intervals (like 6 hours or less).  Two problems have arisen:
1) If it's a short window there sometimes isn't any REM.  This makes it impossible to find the LA since that was where the REM and SWS histograms overlapped. I have modified
make_frequency_plot.m to set LA to the lower end of the SWS histogram if there isn't any REM. 
2) With a moving window of 4 hours, the last two hours of data and the first two hours of data are not fit to the model.  This is OK for long recordings, but for short 
durations it can leave very little data that the model is fit to.  Should this be a parameter that is adjusted based on the length of the recording? FIXED.  I modified 
PROCESSLBATCHMODE.m to use the normal 4-hour window if there is 2 hours of data on either side of the restricted time interval.  If so, go ahead and use it.  The 
lactate model in this case is run only over the restriction requested, but data are used for window_length/2 on either side, allowing UA and LA to cover the entire
restricted region.  This seems to work.   

8:38 lactate plots are now color-coded for sleep state.
9:53 running the delta model on baseline (5.5-8 h) doesn't give any SWS episodes of 5 minutes or longer for some data files.  I have included an error message in 
find_all_SWS_episodes2.m for this case.  Just run baseline on lactate? 

Show Janne all the simulation output.  (lactate during whisking, lactate during new 2.5 hour baseline, lactate during  )
EEG1 and EEG2 run for 2.5 hour recovery, but very little data so fitting is kind of meaningless.
lactate in recovery looks quite good.  Some are beautiful.  None are bad.  

For GitHub: use the Windows GUI and clone the Epoch-Based-Processing repository from Wisorlab.  When I make changes to files 

I'm working on implementing the quiet wake vs. active wake idea.  The code is in Todays_code_testing/Quiet-Active-Wake. 
TO DO: update Franken_like_model.m to use all 4 tau values, not just Ti and Td.  

* Tuesday, February 10, 2015
The model now runs on Janne's data if I use lactate as the input.  However, for 10 out of the 14 files, when run using the entire 
recording as input, Nelder-Mead wasn't able to find a local minimum. It gives a warning about how it ran the maximum number of iterations.  
The output doesn't look great either.  Sometimes when in quiet wake lactate goes up, sometimes it goes down. Most of the time active wake 
means lactate is going up, but not always.  

Running the code on Janne's data using EEG2 also had 12 out of 14 files reach the maximum number of iterations.   Using the entire recording.  

* Thursday, February 12, 2015
Goals for today: 
1) Talk with Jonathan about Janne's project
2) Make a new branch on GitHub and start implementing a GUI for Autoscoring and a GUI for Process L.  Perhaps Autoscoring should be its own thing. 

9:10 I copied importdatafile.m into Brennecke/matlab_pipeline/Matlab/etc/matlab-utils/.  This is where the excel exporting code is and I should be including
this directory in my code anyway.  I didn't want to have several different versions of importdatafile.m floating around, so now there is one copy in matlab-utils.

9:27 Create_TimeStampMatrix_from_textdata.m isn't working on BL-1196  I may have messed it up when I changed it to make it work on Janne's data.  Keep 
looking at this.  
